...
local RemainSelectSpellsSelectors = {
    ["BardMagicalSecrets"] = true
}

local AlwaysPreparedSelectors = {
    ["BardMagicalSecrets"] = true
}
...
local invalid_config_messages = {
    ["boolean"] = '[BardicPreparation] Invalid entry for key "%s" (expected: true or false) in config file, disabling mod'
}
...
local broken_warn_formats = {
    NotSpell = "[BardicPreparation] Spell list %s contained something other than a spell. The bad entry is named %s",
    InvalidSpellLevel = "[BardicPreparation] The spell named %s has an invalid level",
    InvalidSpellContainerID = "T[Bardic Preparation] he spell named %s has an invalid container ID specified",
    MissingSpell = "[BardicPreparation] Spell list %s contained a spell that isn't defined named %s",
    InvalidSpellName = "[BardicPreparation] Spell list: %s contains an invalid spell name",
    InvalidRitualCosts = "[BardicPreparation] The spell named %s has invalid ritual costs",
}
    ...
    for _, progguid in pairs(Ext.StaticData.GetAll("Progression")) do
        ...
        if class_prog_table_ids[pd.TableUUID] or subclass_prog_table_ids[pd.TableUUID] then
            ...
            for _, this_select in ipairs(pd["SelectSpells"]) do
                if AlwaysPreparedSelectors[this_select.SelectorId] then
                    this_select.PrepareType = "AlwaysPrepared"
                end
            end
        end
...
function API_ModifyLists(class_guid, spell_action_resource, filename, Ours, OursRitual)
    ...
    for _, pd in ipairs(prog_data) do
        ...
        for idx, this_select in ipairs(pd["SelectSpells"]) do
            ...
            if this_select.PrepareType == "Unknown" and RemainSelectSpellsSelectors[this_select.SelectorId] == nil then
